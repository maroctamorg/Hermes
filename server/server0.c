...
int open_kavita(state_t * state) {
	...
	*(state->cpid) = fork();
	...
	char* ssh_args[9] = {"/usr/bin/ssh", "-o ServerAliveInterval=300", "-o ServerAliveCountMax=3", "-o ExitOnForwardFailure=yes", "-o StrictHostKeyChecking=no", "-NR", "5000:localhost:5000", "oncoto@oncoto.app", NULL};
	printf("/usr/bin/ssh -o ServerAliveInterval=300 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=no -NR 5000:localhost:5000 oncoto@oncoto.app\n");
...
void close_plex(int signum) {
	...
	char* sshkill_args[4] = {"/usr/bin/ssh", "guimaraes@macMini", "kill `(ps aux | grep \"5001:localhost:32400 oncoto@oncoto.app\"| grep -v grep | awk '{print $2}')`", NULL};
